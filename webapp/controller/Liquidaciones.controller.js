sap.ui.define([
	 ...
	 */
	function (Controller, JSONModel, Dialog, Library, Core, Button, Label, TextArea, MessageBox) {
		"use strict";
		return Controller.extend("enap.com.zapp_hr_get_liq.controller.Liquidaciones", {
			...
			onButtonPressDownloadPDF: function (oEvent) {

				var link = this.getViewModel("oViewModel").getProperty("/urlPdf");
				$.ajax({
					url: link,
					method: "GET",
					success: function (oData, status, xhr) {
						...
						if (contentType === "application/pdf") {
							...
							MessageBox.error("Tipo de Contenido no es un PDF");
							console.error(xhr.responseText);
						}
					},
					error: function (xhr, status, error) {
						//MessageBox.error(xhr.responseXML.getElementsByTagName("message")[0].getHTML());
						MessageBox.error("PDF no puede ser cargado o no existe");
						console.error(xhr.responseText);
					}
				});
